...
public final class DatabaseHelper {
    ...
    private static String getTableName(Class<?> entityClass) {
        String tableName = "";
        for (int i=0; i<entityClass.getSimpleName().length(); i++) {
            if (Character.isUpperCase(entityClass.getSimpleName().charAt(i))) {
                tableName += (i==0 ? "" : "_") + Character.toLowerCase(entityClass.getSimpleName().charAt(i));
            } else {
                tableName += entityClass.getSimpleName().charAt(i);
            }
        }
        return tableName;
    }
   ...
   /* private static String getTableName(Class<?> entityClass) {
    ...
    public static void executeSqlFile(String filePath) {
        //从当前线程中获取线程上下文中的classloader, 通过classpath下的filePath获取资源
